...
```text
07:30 每日任务
    -> IBKR-Statement 最新可用且可证明同批的持仓快照
...
真实索引 schema 没有持久化的 import success 状态。设计不虚构成功表或成功字段：账户选择必须无歧义，`account_id + report_date + source_query_type + source_file_name` 四键全部非空，position query 同时精确过滤并逐文档后验验证四键。根据 transformer 先账户、后持仓且分别生成 `ingested_at` 的实际行为，持仓时间还必须不早于账户时间且相差不超过 5 分钟；不能证明同批即返回 `incomplete`，正式覆盖保持未验证并停止仓位动作。`document_batch_time` 只表示文档批次时间，不冒充导入成功审计时间。
...
- 没有最新可用且可证明同批的 IBKR 快照时，用户或题设提供的证券仅为临时范围；正式覆盖显示“未验证”，不得生成 coverage 分数、仓位动作或声称全持仓覆盖。
...
每日先用确定性规则做全持仓扫描，再选择最多 6 个需要展开的持仓。新闻时间窗优先从上一份成功生成晨报的新闻截止时间续接；无记录时正常连续交易日回看 36 小时，星期一或 NYSE 休市后的首份晨报最多回看 96 小时，间隔超过 96 小时须披露未覆盖缺口。输入最多扫描 120 条候选元数据，去重为最多 40 个事件簇，只深读最多 12 个事件簇；单事件默认最多 3 源，达到双源门槛即停止，冲突时最多额外 2 源。实际持仓直接事件与系统性事件优先进入元数据扫描；优先队列本身溢出时披露并取消完整覆盖声明。最终只做一次组合级综合分析；只有财报命中且确实需要深挖时，允许增加一次专项分析。报告限制为重大市场事件 5 条、科技事件 4 条、重点持仓 6 个，其余合并为“无重大变化”。

## 持仓建议纪律

- 必须使用最新可用且可证明同批的实际持仓及其文档批次时间；数据缺失、无法证明同批或过期时明确提示，不能猜测。
